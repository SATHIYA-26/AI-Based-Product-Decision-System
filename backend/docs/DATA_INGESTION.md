...
2. **Upload reviews:**
```bash
curl -X POST -F "file=@reviews.csv" -F "source=play_store" http://localhost:5000/upload-csv
```

3. **Ingest via API:**
```bash
curl -X POST -H "Content-Type: application/json" \
  -d '{"reviews":[{"text":"App crashes","rating":1}],"source":"app_store"}' \
  http://localhost:5000/ingest-api
```
...
gunicorn -w 4 -b 0.0.0.0:5000 -m 755 "from run import app; app" 
rm app.main:app
```

Or use the run.py wrapper:
```bash
gunicorn -w 4 -b 0.0.0.0:5000 run:app
```

2. **Use a production database (PostgreSQL):**
Update the database URL in `app/models/ingestion.py` and `app/models/cluster_analysis.py`:
```python
...
4. **Important:** Ensure all package directories have `__init__.py` files:
- `app/__init__.py` ✅
